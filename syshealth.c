...
int main(int argc, char *argv[])
{
	...
	zombie_check = uptime_check = df_H_check = free_check = 0;
	vmstat_check = users_check = lsof_check = loadavg_check = 0;
	diskstat_check = netstat_s_check = netstat_naptu_check = 0; 
	top_check = iotop_check = ps_axl_check = ps_axu_check = 0;

	logdir[0]='\0';

	while ((opt = getopt(argc, argv, "hd:zUHfVuovsnNtTlx")) != -1) {
	        switch (opt) {
	        case 'h':
	        	bHelp = 1;
	            break;
	...
	sprintf(&tname[0],"%s%s.tmp",logdir, argv[0]);
	sprintf(buf,"%s%s_%%m%%d",logdir, argv[0]);
